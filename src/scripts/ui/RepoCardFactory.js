...
export class RepoCardFactory {
   ...
   * @param {string} repo.name - The title of the repository.
   ...
   * @param {string} repo.imageSrc - The source path or URL for the image.
   * @param {string} repo.pagesLink - The URL to the live site.
   * @param {string} repo.repoLink - The URL to the source repository.
    ...
    li.innerHTML = `
      <div class="flex flex-col p-6">
          <div class="flex-grow mb-4">
              <span class="repo-name repo-name-font block w-full mb-3 font-bold">${repo.name}</span>
          ...
          <div class="repo-image-container relative overflow-hidden max-h-0 opacity-0 invisible transition-all duration-300 ease-in-out mt-0 mb-4 w-full bg-black rounded-xl border border-cyan-500/40 shadow-[0_0_20px_rgba(0,255,255,0.15)]">
              <div class="relative group">
                  <img alt="${repo.imageAlt}" src="${repo.imageSrc}" data-src="${repo.imageSrc}" class="w-full h-auto cursor-pointer transition-transform duration-500 group-hover:scale-105 repo-image-trigger" title="Click to enlarge">
          ...
          <div class="repo-links-container flex flex-wrap items-center justify-between gap-4 mt-auto pt-4 border-t border-[var(--blue-5)]/20 w-full">
              <div class="flex gap-4">
                  <a href="${repo.pagesLink}" target="_blank" class="flex items-center gap-2 text-slate-400 hover:text-cyan-400 hover:translate-x-1 hover-pulse-blue hover:underline transition-all duration-300">
                      <i class="fas fa-link"></i> <span class="hidden sm:inline italic">Visit Site</span>
                  </a>
                  <a href="${repo.repoLink}" target="_blank" class="flex items-center gap-2 text-slate-400 hover:text-cyan-400 hover:translate-x-1 hover-pulse-blue hover:underline transition-all duration-300">
